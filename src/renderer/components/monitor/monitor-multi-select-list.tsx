	...
	return (
		<MenuList>
			{monitors.map((monitor) => (
				<MenuItem key={`multi-select-item-${monitor.id}`} selected={selected(monitor)} onClick={() => toggleItemSelected(monitor)}>
					<ListItemIcon>{selected(monitor) ? <RadioButtonChecked /> : <RadioButtonUnchecked />}</ListItemIcon>
					<ListItemText
						primary={<Typography textTransform={"capitalize"}>{monitor.nickname}</Typography>}
						secondary={
