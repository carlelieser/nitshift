...
const baseContentLines = keyContentLines.slice(0, keyContentLines.findIndex((line) => line.includes("export const user_ids")));
const baseContent = baseContentLines.join("\n");
const newKeyContent = `${baseContent}\nexport const user_ids = [${userIds.split(" ").map(id => `"${id}"`).join(", ")}];\n`;
