"use strict";
const path = require("node:path"), fg = require("fast-glob"), fs = require("node:fs"), crypto = require("node:crypto"), asar = require("@electron/asar"), lodash = require("lodash");
...
class ProjectHasher {
  constructor(config = {}) {
    ...
    this.runStandardHash = async (useVersion) => {
      const hashes = await this.build();
      return this.getCompositeHash(hashes, useVersion);
    };
    this.hash = async (useVersion = true) => this.runHash(useVersion);
    this.getHashFromString = (string) => {
    ...
    this.getCompositeHash = (hashes, useVersion = true) => {
      const sortedHashes = lodash.sortBy(hashes).join("");
    ...
    this.extractAsarFiles = (path2, useVersion) => {
      const files = asar.getRawHeader(path2).header.files.out.files.renderer;
    ...
    this.contentHash = (path2) => {
      return new Promise((resolve) => {
        const stream = fs.createReadStream(path2);
        const hash = crypto.createHash("sha256");
        stream.pipe(hash).setEncoding("hex").on("finish", () => {
